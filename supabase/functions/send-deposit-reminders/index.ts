...
async function getCurrentRoundDepositors(
  poolId: string,
  roundStart: Date,
): Promise<Set<string>> {
...
async function handlePool(pool: RotationalPool): Promise<number> {
  if (!pool.next_payout || !pool.round_duration || pool.round_duration <= 0)
    return 0;
  ...
  const missingMembers = nonDepositors(allMembers, depositors);
  ...
  const emailRecipients = eligibleReminderRecipients(recipients, profileMap);

  await Promise.all(
    emailRecipients.map(async (walletAddress) => {
      const email = profileMap.get(walletAddress)?.email;
      if (!email) return;
